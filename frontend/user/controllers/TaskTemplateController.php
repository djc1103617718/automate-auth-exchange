...
class TaskTemplateController extends UserBaseController
{
    // 控制器下的操作类型
    const TYPE_CREATE = 1;
    ...
    public function actionDraftApp()
    {
        $appList = TaskTemplateForm::appIdToName();
        return $this->render('draft-app', [
            'appList' => $appList
        ]);
    }
     ...
     * @throws Exception
     */
    public function actionDraftJob($id)//action_id
    {
        $appAction = AppAction::findOne(['action_id' => $id, 'status' => AppAction::STATUS_NORMAL]);
        $actions = AppAction::find()->where(['app_id' => $appAction->app_id, 'status' => AppAction::STATUS_NORMAL])->asArray()->all();
        $inputArray = TaskTemplateForm::formatJobParamForView($id);
     ...
     * @throws Exception
     */
    public function actionDraftJobProcess($action_id)
    {
        $model = new TaskTemplateForm();
        if ($model->load(Yii::$app->request->post()) && $model->draftSave()) {
            return $this->redirect(['job/draft-index']);
        } else {
            $errorMsg = array_values($model->errors);
            Yii::$app->session->setFlash('error', $errorMsg[0]);
            return $this->redirect(['draft-job', 'id' => $action_id, 'model' => $model]);
        }
    }
     ...
     * @throws Exception
     */
    public function actionDraftUpdate($id)//job_id
    {
        $model = new TaskTemplateForm();
        $model->setScenario('draft-update');
        $jobModel = $this->findOneModel($id);
        $model->setAttributes(ArrayHelper::toArray($jobModel));

        if ($model->load(Yii::$app->request->post()) && $model->update()) {
            return $this->redirect(['job/draft-index']);
        } else {
            $appActionModel = AppAction::findOne(['action_name' => $jobModel['job_name'], 'status' => 1]);
            $app = App::findOne($appActionModel['app_id']);
            $inputArray = TaskTemplateForm::formatJobParamForUpdate($appActionModel['action_id'], $id);
            if (empty($inputArray)) {
                throw new Exception('自动化步骤参数缺失!');
            }
            $actionPrice = $this->getPublicPriceList($appActionModel->action_class_name);
            $templateFileName = $this->getTemplateFileName($appActionModel->category, self::TYPE_DRAFT_UPDATE);
            $errorMsg = array_values($model->errors);
            Yii::$app->session->setFlash('error', $errorMsg[0]);
            return $this->render($templateFileName, [
                'app' => $app,
    ...
    public function actionDraftToTask($id)//$job_id
    {
        $model = $this->findOneModel($id);
        if (!$model->draftToTask()) {
            Yii::$app->session->setFlash('error', '提交新任务失败!');
            return $this->redirect(['job/view', 'id' => $id]);
        }
        Yii::$app->session->setFlash('success', '提交任务成功!');
        return $this->redirect(['job/view', 'id' => $id]);
    }
