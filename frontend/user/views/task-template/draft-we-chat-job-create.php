...
<div style="position:relative; left:50%;width:100%;">
    <div style="position:relative; left:-50%;width:100%;text-align: center;">
        ...
        <div style="float:left;position:relative;width:30%;margin-left: 30px; ">
            <p><a class="btn btn-primary" style="width:80%;" href="<?= Url::to(['task-template/draft-app'])?>">返回</a></p>
            <?php
            if ($actions) {
                foreach ($actions as $item) {
                    ?>
                    <p><a class="btn btn-primary" style="width:80%;" href="<?= Url::to(['task-template/draft-job', 'id' => $item['action_id']])?>"><?= $item['action_name'] ?></a></p>
                    <?php
                }
            } ?>
        </div>
        <div style="float:left;position:relative;width:60%;margin-left: 10px; border-right:dashed #d2d6de 1px; padding-left:20px;" >
...
<script type="text/javascript">
    ...
    $('#reset-form').click(function(){
        var url = "<?= Url::to(['reset-task', 'action_id' => $action_id, 'type' => 1]);?>";
        $('#job_create_form').attr('action', url);
        $('#job_create_form').submit();
    })
</script>